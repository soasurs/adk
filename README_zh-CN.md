...
`llmagent.InstructionProvider` 在每次 LLM 调用前生成一条临时 system instruction。
它的输出只影响当前请求，不会被持久化。API 契约、生命周期、缓存取舍和示例请参阅
[动态 System Instruction](docs/dynamic-instruction_zh-CN.md)。
...
ADK 可以归档旧 session events，但不会生成摘要，也不会决定何时归档。摘要的生成、
存储和注入策略完全由应用负责。

```go
events, _ := sess.ListEvents(ctx)

boundaryEventID := events[4].EventID // 第一个要保留的 event
_ = sess.ArchiveEventsBefore(ctx, boundaryEventID)

archived, _ := sess.ListArchivedEvents(ctx)
```

传入零会归档全部 active events。非零边界必须是 active event，且该 event 本身仍保持
active。归档不会创建或删除 event。partial events 不会被持久化，因此也无需归档。
应用需要自行协调归档操作与并发运行。

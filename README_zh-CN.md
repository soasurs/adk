...
当 `LlmAgent` 需要在每次 LLM invocation 前重新生成临时 system instruction 时，
可以使用 `InstructionProvider`：
...
Provider 接收排除 system message 的 conversation 独立副本。它的输出只在当前请求中
与静态 instruction 和最新 compaction summary 组合，不会被 yield、持久化，也不会
由 SDK 缓存。这是动态 system instruction hook，不是 context storage。稳定事实通常
应通过 user、assistant 或 tool events 表达。

Provider 输出发生变化时，从首个不同 token 开始会降低 prompt-cache 复用。重视缓存
稳定性时，应避免不必要的时间戳、随机值和逐轮变化内容。ADK 不自动缓存 Provider
输出，也不实现 provider-specific cache controls；调用方自行权衡动态指导与稳定前缀。

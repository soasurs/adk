...
## 项目结构
ADK 将“代理”抽象为统一接口，围绕该接口构建了多种具体代理实现：
- 核心接口定义位于 agent 包
- 具体实现包括基于 LLM 的代理、顺序编排代理、并行编排代理
- runner 负责与会话服务集成，驱动代理执行并持久化消息
- examples 展示了端到端的聊天应用示例

```mermaid
graph TB
subgraph "代理层"
A["Agent 接口<br/>agent/agent.go"]
L["LLM 代理<br/>agent/llmagent/llmagent.go"]
S["顺序代理<br/>agent/sequential/sequential.go"]
P["并行代理<br/>agent/parallel/parallel.go"]
end
subgraph "运行时"
R["Runner<br/>runner/runner.go"]
M["模型与事件<br/>model/model.go"]
T["工具接口<br/>tool/tool.go"]
end
subgraph "示例"
E["聊天示例<br/>examples/chat/main.go"]
end
A --> L
A --> S
A --> P
...
- [main.go:101-125](file://examples/chat/main.go#L101-L125)

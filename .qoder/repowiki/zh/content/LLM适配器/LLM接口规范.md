...
本规范文档详细阐述了LLM接口的设计理念、数据结构定义、实现要求以及最佳实践，为开发者提供完整的技术指导。
...
| 配置项 | 类型 | 描述 | 默认值 |
|--------|------|------|--------|
| Temperature | float64 | 控制生成随机性 | 0.0（使用提供商默认） |
| ReasoningEffort | ReasoningEffort | 推理努力级别 | ""（不设置） |
| ServiceTier | ServiceTier | 服务层级 | ""（自动选择） |
| MaxTokens | int64 | 最大生成令牌数 | 0（使用提供商默认） |
| ThinkingBudget | int64 | 思维预算 | 0（使用提供商默认） |
| EnableThinking | *bool | 启用内部思考 | nil（交由提供商决定） |